...
Function Get-ADComputerSettings
{
    # CmdletBinding zorgt ervoor dat deze functie wordt behandelt alsof het een echte PowerShell cmdlet is.
    [CmdletBinding()]

    $arrADComputerProperties=@()

    Process
    {
        $strADComputerSettings = Get-Content -Path $FilePathADComputerSettings

        $arrADComputerSettings = $strADComputerSettings.Split([Environment]::NewLine, [StringSplitOptions]::RemoveEmptyEntries)

        ForEach($ADComputerSetting in $arrADComputerSettings)
        {
            If($ADComputerSetting.Contains("TRUE"))
            {
                $ADComputerSetting = $ADComputerSetting -replace " = TRUE", ","
                $arrADComputerProperties += $ADComputerSetting
            }
    
        }
        
        $ADComputerSettingLoaded = $true
    }    
}